...
export class V1VacuumFeatures extends BaseDeviceFeatures {
	...
	public async detectAndApplyRuntimeFeatures(statusData: Readonly<Record<string, any>>): Promise<boolean> {
		...
		// Some firmwares/devices (e.g. a97) deliver 'dss' but omit 'dock_type' in early/partial status payloads.
		// In that case we still want the docking-station states to exist and be updated.
		if (statusData["dss"] !== undefined && !("dock_type" in statusData)) {
			if (await this.applyFeature(Feature.DockingStationStatus)) changed = true;
		}
	...
	protected async initDockingStationStatus(): Promise<void> {
		...
		const commonStates: Record<number, string> = {
			0: "UNKNOWN",
			1: "ERROR",
			2: "OK"
		};
